...
*****************************************************************************/
#include "../include/NMPC_solver_rpy.h"

NMPC_solver_rpy::NMPC_solver_rpy(/* args */) {
  LegForces.setZero();
  init_flag = 1;
}

void NMPC_solver_rpy::run(const Matrix<double, robot_state_dim, 1> &state,
...
NMPC_solver_rpy::~NMPC_solver_rpy() {
  if (thread_nmpc.joinable())
  {
    thread_nmpc.join();
  }
}
