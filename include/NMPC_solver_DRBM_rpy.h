...
class NMPC_solver_DRBM_rpy {
 ...
 public:
  ...
  Matrix<double, 3, 1> W_orientation_object;
  ...
  void computeContactForces(const Matrix<double, robot_state_dim+object_state_dim, 1>& state,
                            ...
                            const Mat43& ToePos_wB, const Vector3d& Object_Pos,
    ...
    if (nmpcEnable && init_flag) {
      ...
      thread_nmpc.detach();
      cout << "NMPC_DRBM_rpy thread is created!" << endl;
    }
  }
   ...
   */
  void run(const Matrix<double, robot_state_dim+object_state_dim, 1>& state,
           const Matrix<double, robot_state_dim+object_state_dim, 1>& state_d, const Mat43& footpos,
           const Vector3d &objectpos, const Vector4i& contact_state,const Matrix<double, 6, 1> arm2body_Forces);
